...
def train_mnist():
    ...
    n_epoch = 20 # 用于训练模型的纪元数
    batch_size = 256 # 在训练期间一次处理的数据点（即本例中的图像）的数量
    n_T = 400  # 500 从模型生成样本时要采取的扩散步骤数
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    n_classes = 10 # 数据集中的类数
    n_feat = 128  # 128 ok, 256 better (but slower)
    lrate = 1e-4 #优化器在训练期间使用的学习率
    save_model = False
    ...
    tf = transforms.Compose([transforms.ToTensor()])  # 将图像转换为张量，并将图像的像素值缩放到0到1之间 mnist is already normalised 0 to 1

    dataset = MNIST("./data", train=True, download=True, transform=tf) #加载数据集
    ...
    for ep in range(n_epoch):
        ...
        with torch.no_grad():
            ...
            for w_i, w in enumerate(ws_test):
                ...
                x_all = torch.cat([x_gen, x_real]) # 各个数字的图片
                grid = make_grid(x_all * -1 + 1, nrow=10) # 把数字图片组合
                # print("grid: ")
                # print(grid)
                save_image(x_all[0][0], save_dir + f"image_ep{ep}_w{w}.png")
